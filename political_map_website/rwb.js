...
UpdateMapById = function (id, tag) {
  ...
  NewData = function (data) {
    ...
    console.log(document.getElementById("democ_comm_comm_amount") != null)
    if(document.getElementById("democ_comm_comm_amount") != null){
      TransColoring()
    }
    if(document.getElementById("opinion_color_stats_summary") != null){
      OpinionsColoring()
    }
  ...
  ViewShift = function () {
    ...
    if ($("#4-fuq-sake")) {
      console.error("This should be here");
    }
    if ($("#print-table")) {
      console.error("printing table");
    }
    if ($("#execsql-returns-error")) {
      console.error("EXESQL ERROR!");
    }
  ...
  TransColoring = function(){
    var democ_comm_amount = document.getElementById("democ_comm_comm_amount").innerHTML;
    if (democ_comm_amount == null){
      democ_comm_amount = 0
    }
    var democ_cand_amount = document.getElementById("democ_comm_cand_amount").innerHTML;
    if (democ_cand_amount == null){
      democ_cand_amount = 0
    }
    var democ_count = Number(democ_comm_amount) + Number(democ_cand_amount);
    
    var repub_comm_amount = document.getElementById("repub_comm_comm_amount").innerHTML;
    if (repub_comm_amount == null){
      repub_comm_amount = 0
    }
    var repub_cand_amount = document.getElementById("repub_comm_cand_amount").innerHTML;
    ...
    var summary = document.getElementById("comm_transfer_summary");
    if (difference > 0) {
  ...
  OpinionsColoring = function(){
    var opinion_numbers = document.getElementById("opinion_color_stats_summary").innerHTML;
    var vals = opinions_numbers.split(" ");
    var mean_str = opinions_numbers[0];
    var mean = Number(mean_str)
    
    if (mean > 0) {
      opinion_numbers.style.backgroundColor = "blue";
    }
    if (mean < 0 ) {
      opinion_numbers.style.backgroundColor = "red";
    }
  },
...
Start = function (location) {
  ...
  var opinion_url = 'https://murphy.wot.eecs.northwestern.edu/~yhl4722/rwb/rwb.pl?' + 'act=give-opinion-data&latitude=' + String(lat) + '&longitude=' + String(long);
...
getWhatArray = function () {
  var what_arr = []
  var what_checkbox_division = document.getElementById('what_checkbox_division')
  ...
  for (var i = 0, what_checkbox_num = what_checkboxes.length; i < what_checkbox_num; i++) {
...
getCyclesArray = function () {
  var cycles_arr = []
  var cycle_checkbox_division = document.getElementById('cycle_checkbox_division')
  ...
  for (var i = 0, cycle_checkbox_num = cycle_checkboxes.length; i < cycle_checkbox_num; i++) {
