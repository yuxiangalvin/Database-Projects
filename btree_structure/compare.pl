...
if ($numerr<$maxerrs) {
    # student's code could have crashed early... 
    if (!eof(CMD)) { 
      print "----------------------------------------------------------------------------\n";
      print "ERROR: testing completed while there are remaining operations\n";
      print "It is likely that your implementation crashed or quit!\n";
      print "Last operation handled was \"$cmd\"\n\n";
      print "The following (up to) 10 unhandled commands are:\n";
      for ($count=0;($count<10) && (!eof(CMD));$count++) { 
	  $cmd = <CMD>; chomp($cmd);
	  print "\t$cmd\n";
      }
      print "----------------------------------------------------------------------------\n";
